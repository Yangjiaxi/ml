...
class FC(Layer):
    def __init__(self, W, b, lr, delta, rho1, rho2, decay, epoch_drop):
        self.W = W.copy()
        self.b = b.copy()
    ...
    def backprop(self, back_grad):
        ...
        self.sW = self.rho1 * self.sW + (1 - self.rho1) * gW
        self.rW = self.rho2 * self.rW + (1 - self.rho2) * gW ** 2
        self.sb = self.rho1 * self.sb + (1 - self.rho1) * gb
        self.rb = self.rho2 * self.rb + (1 - self.rho2) * gb ** 2
    ...
    def update(self):
        self.lr_update()
        sW = self.sW / (1 - self.rho1 ** self.count)
        rW = self.rW / (1 - self.rho2 ** self.count)
        sb = self.sb / (1 - self.rho1 ** self.count)
        rb = self.rb / (1 - self.rho2 ** self.count)
        dW = -self.lr * sW / (np.sqrt(rW) + self.delta)
        db = -self.lr * sb / (np.sqrt(rb) + self.delta)
        self.W += dW
        self.b += db
